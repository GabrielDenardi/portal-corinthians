...
export async function MatchCard({ match }: MatchCardProps) {
  const [homeTeamCrest, awayTeamCrest] = await Promise.all([
    match.homeTeamBadgeUrl ? Promise.resolve(match.homeTeamBadgeUrl) : getClubCrestUrl(match.homeTeam),
    match.awayTeamBadgeUrl ? Promise.resolve(match.awayTeamBadgeUrl) : getClubCrestUrl(match.awayTeam),
  ]);
